...
if(!Array.isArray(promos))return res.status(400). json({error:'Invalid data'});
...
return res.status(200).json({success:true,message:`${promos.length} promos reçues`,count:promos.length});
}catch(error){
